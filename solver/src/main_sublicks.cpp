...
int main(int argc, char *argv[])
{
    // Parameter reading
	if (argc != 19){
		cout << "\nProblem with the number of parameters\n";
		return 0;
	}

    // User specifications
	int n_bend = atoi(argv[1]);
	double r_bend = atof(argv[2]);

	int n_bn = atoi(argv[3]);
	double r_bn = atof(argv[4]);

	int n_slide = atoi(argv[5]);
	double r_slide = atof(argv[6]);

	int n_tri = atoi(argv[7]);
	double r_tri = atof(argv[8]);

	int n_tri_bend = atoi(argv[9]);
	double r_tri_bend = atof(argv[10]);

	int n_tri_bn = atoi(argv[11]);
	double r_tri_bn = atof(argv[12]);

	int n_tri_rest = atoi(argv[13]);

	int n_rest = atoi(argv[14]);

	int n_dStop = atoi(argv[15]);

	int n_note = atoi(argv[16]);
	double r_note = atof(argv[17]);

	string sl_type = argv[18];

	char arr[100];
	string lp_path = sl_type + "_licks/sublicks.lp";
	string log_path = sl_type + "_licks/sub.log";
    
    // Number of sublicks
	int nSubs;
	ifstream nSub(sl_type + "_licks/files/nSub");
	nSub >> nSubs;
    ...
    (*sptrPData).numPoints = nSubs;
    (*sptrPData).costMatrix = costMatrix;
	...
	sublicks(n_bend, r_bend, 
			 n_bn, r_bn,  
			 n_slide, r_slide, 
			 n_tri, r_tri, 
			 n_tri_bend, r_tri_bend, 
			 n_tri_bn, r_tri_bn, 
			 n_tri_rest, 
			 n_rest, 
			 n_dStop, 
    ...
    model = CPXcreateprob(env, &status, "SublicksProblem");

	strcpy(arr, lp_path.c_str());
    CPXreadcopyprob(env, model, arr, NULL);
    ...
    ofstream route_file;
    route_file.open(sl_type + "_licks/route");
