...
int main(int argc, char *argv[])
{
	...
	double normal_rate = atof(argv[1]);
	double rest_rate = atof(argv[2]);
	double repetition_rate = atof(argv[3]);
	double turnaround_rate = atof(argv[4]);
	double bn_rate = atof(argv[5]);

    // Number of licks
	int nLicks;
	ifstream nLick("gs_optimizer/solver/files/nLicks");
	nLick >> nLicks;
	...
	double ** cMatrix = new double *[nLicks];
	
	for (int i = 0 ; i < nLicks ; i++)
		cMatrix[i] = new double[nLicks];
	
	cMatrix = cost_matrix(nLicks, "gs_optimizer/solver/files/matrix");

    sptrPData = new ProblemData;
    (*sptrPData).numPoints = nLicks;
    (*sptrPData).costMatrix = cMatrix;
    ...
    //CreateLP(sptrPData);
	licks(nLicks, normal_rate, rest_rate, repetition_rate, turnaround_rate, bn_rate);
    ...
    CPXreadcopyprob(env, model, "gs_optimizer/solver/licks.lp", NULL);
    ...
    CPXFILEptr fp;
    fp = CPXfopen("gs_optimizer/solver/lick.log", "w");
    ...
    ofstream route_file;
    route_file.open("gs_optimizer/solver/route");
